...
class News
{
	public function showNewsList()
	{
		...
		$cnt = DB::result_first("SELECT COUNT(*) FROM ".DB::table('kfss_news'));
		if ( $cnt > 0 )
		{
			...
			$newsdb = array();
			$query = DB::query("SELECT * FROM ".DB::table('kfss_news')." ORDER BY addtime DESC LIMIT $start, $readperpage");
			$i = 0;
		...
		include template('simstock:news');
	}
	public function showNewsInfo($id=0)
	{
		global $baseScript, $_G, $db_smname, $hkimg;
		require libfile('function/discuzcode');
		$news = DB::fetch_first("SELECT * FROM ".DB::table('kfss_news')." WHERE nid='$id'");
		$news['content'] = discuzcode($news['content']);
		$news['addtime'] = dgmdate($news['addtime']);
		include template('simstock:news_info');
	}
	public function getLatestNews($num=0)
	{
		global $baseScript;
		$newsList = array();
		$query = DB::query("SELECT * FROM ".DB::table('kfss_news')." ORDER BY addtime DESC LIMIT 0,$num");
