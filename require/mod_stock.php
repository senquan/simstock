...
class Stock
{
	...
	public function getStockholdersList($code, $totalnum, $start, $readperpage)
	{
		...
		if ( $code )
		{
			$query = DB::query("SELECT uid, username, stocknum_ava, averageprice FROM ".DB::table('kfss_customer')." WHERE code='$code' ORDER BY stocknum_ava DESC LIMIT $start, $readperpage");
